    ...
    try {
        if(file) {
            ...
            if (mime.startsWith('image/')) {
                ...
                await upsertToIndex({ vectors: [vector], type: 'image', filename, mimeType: mime });
                
            } else if (mime.startsWith('video/')) {
                
                console.log('embedding video');
                const vectors = await embedVideo(file);
                await upsertToIndex({ vectors, type: 'video', filename, mimeType: mime });
...
app.post('/chat', upload.single('content'), async (req, res) => {
    try {
        const input = req.file || req.body.content;
